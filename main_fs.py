...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for name, param in model.named_parameters():
        # 如果是底层骨干网络 (Stratified Transformer)，严格冻结
        if "encoder" in name:
            param.requires_grad = False
        # 其余的模块 (agglayers, cls, kpconv, geo_branch, sem_branch 等) 全部放开！
        else:
...
def validate(val_loader, model, valid_calsses):
    ...
    for i, batch in enumerate(val_loader):
        ...
        if args.forvis:
            ...
            if getattr(args, "export_evidence", False) and hasattr(net, "evidence_cache") and len(net.evidence_cache) > 0:
                cache = net.evidence_cache[0]
                np.savez_compressed(
                    os.path.join(save_dir, "evidence.npz"),
                    corr_geo=cache["corr_geo"].numpy(),
                    corr_sem=cache["corr_sem"].numpy(),
                    alpha=cache["alpha"].numpy(),
                    corr_fused=cache["corr_fused"].numpy(),
                )           
            torch.cuda.empty_cache()
